...
class DiscreteMultiAgentTrainingEnv_2D_v0:

    def __init__(self, width=10.0, height=10.0, agent_init_pos=None, agent_radius=None, obstacle_pos=None,
                 obstacle_radius=None, task_pos=None, task_radius=None, accuracy=1.0, multi_step=10, gamma=0.99):
        """
        2D 离散多智能体训练环境
        Attributes:
            width (int): 环境宽度。
        ...
        self.agent_action = np.zeros((self.agent_pos.shape[0], 1), dtype=int)   # self.agent_action：agent的动作编号
    ...
    def reset(self, mode=1, new_agent_init_pos=None):
        ...
        self.agent_action = np.zeros((self.agent_pos.shape[0], 1), dtype=int)
    ...
    def explore_action(self):
        agent_vx = np.random.randint(-1, 2, size=(self.agent_v.shape[0], 1))
        agent_vy = np.random.randint(-1, 2, size=(self.agent_v.shape[0], 1))
        self.agent_v = np.hstack((agent_vx, agent_vy)) * self.accuracy
        # print(self.agent_v)
        self.agent_action = 5 + agent_vx + 3 * agent_vy
    ...
    def save_experience(self):
        self.agent_pos_memory.append(self.agent_pos.flatten())
        self.task_state_memory.append(self.unfinished_tasks.T)
        self.agent_action_memory.append(self.agent_action.flatten())
    ...
    def step(self):
        ...
        if if_trajectory_reward:
            task_state_1_decimal = np.array(np.dot(self.task_state_memory[0], 2 ** np.arange(self.task_state_memory[0].size)[::-1]),dtype=int)
            task_state_n1_decimal = np.array(np.dot(self.task_state_memory[-1], 2 ** np.arange(self.task_state_memory[-1].size)[::-1]),dtype=int)
            return (self.agent_pos_memory[0], self.agent_pos_memory[-1], task_state_1_decimal, task_state_n1_decimal,
                    self.agent_action_memory[0], self.trajectory_reward)

        else: return np.array([]), np.array([]), np.array([],dtype=int), np.array([],dtype=int), np.array([],dtype=int), np.array([])
