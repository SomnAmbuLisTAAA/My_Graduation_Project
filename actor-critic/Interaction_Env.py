...
class Interaction_Env:
    ...
    def __init__(self, width=10.0, height=10.0, agent_init_pos=None, agent_radius=None, obstacle_pos=None,
                 obstacle_radius=None, task_pos=None, task_radius=None, accuracy=0.25, multi_step=5, gamma=0.95, device='cuda'):
        """
        2D 离散多智能体训练环境
        Attributes:
            width (int): 环境宽度。
        ...
        self.dis_agent2agent_memory.append(self.dis_agent2agent[self.dis_agent2agent_mask].reshape(self.dis_agent2agent.shape[0], -1).copy())

        self.trajectory_reward = np.zeros((self.agent_pos.shape[0], 1),dtype=float)    # 存储一个multi_step的路径回报
        ...
        self.action_function = [self.explore_action, self.act_by_policy]
        self.probabilities = [0.90, 0.10]
    ...
    def reset(self, mode=1, new_agent_init_pos=None):
        ...
        self.trajectory_reward = np.zeros((self.agent_pos.shape[0], 1),dtype=float)
    ...
    def explore_action(self):
        ...
        vx = np.random.uniform(-1.0, 1.0, size=n)
        vy = np.random.uniform(-1.0, 1.0, size=n)
        self.agent_v = np.column_stack((vx, vy)) * self.accuracy'''

        agent_v = np.random.randint(-2, 3, size=(self.agent_v.shape[0], 2))
        self.agent_v = agent_v * self.accuracy
        # print(self.agent_v)
    ...
    def save_experience(self):
        ...
        self.task_state_memory.append(self.unfinished_tasks.T.copy())
        self.agent_action_memory.append(self.agent_v.reshape(1,-1))
        # print(self.agent_action_memory)
        self.immediate_reward_memory.append(self.immediate_reward.copy())
        self.dis_agent2agent_memory.append(self.dis_agent2agent[self.dis_agent2agent_mask].reshape(self.dis_agent2agent.shape[0], -1).copy())
        ...
        if len(self.immediate_reward_memory) == self.immediate_reward_memory.maxlen:
            immediate_reward_memory_np_array = np.hstack(self.immediate_reward_memory)
            # print(immediate_reward_memory_np_array)
            # print(self.gamma_powers)
            # print(immediate_reward_memory_np_array * self.gamma_powers)
            self.trajectory_reward = np.sum((immediate_reward_memory_np_array * self.gamma_powers),axis=1)
    ...
    def step(self, mode = 'default'):
        ...
        if mode == 'default':
            random.choices(self.action_function, weights=self.probabilities, k=1)[0]()
        else:
            self.explore_action()
        ...
        if if_trajectory_reward:
            ...
            observation_1_matrix = np.hstack((self.agent_pos_memory[0], np.tile(task_state_1_decimal, (self.agent_pos.shape[0], 1))))

            observation_n1_matrix = np.hstack((self.agent_pos_memory[-1], np.tile(task_state_n1_decimal, (self.agent_pos.shape[0], 1))))

            state_1_matrix = np.hstack( (self.agent_pos_memory[0].reshape(1,-1), task_state_1_decimal, self.agent_action_memory[0]) )
            state_1_matrix_without_actions = np.hstack( (self.agent_pos_memory[0].reshape(1,-1), task_state_1_decimal) )
            state_n1_matrix_without_actions = np.hstack( (self.agent_pos_memory[-1].reshape(1,-1), task_state_n1_decimal) )

            return (observation_1_matrix, observation_n1_matrix,
                    state_1_matrix,
                    state_1_matrix_without_actions, state_n1_matrix_without_actions,
                    self.trajectory_reward.reshape(1,-1))

        else: return np.array([]), np.array([]), np.array([]), np.array([]), np.array([]), np.array([])
    ...
    def load_actor(self, actor1, actor2):
        self.actor_network_1.load_state_dict(actor1.state_dict())
        self.actor_network_2.load_state_dict(actor2.state_dict())