...
from Replay_Buffer import ReplayBufferDeque
...
writer = SummaryWriter(log_dir="runs/0226_1552")
...
buffer = ReplayBufferDeque(buffer_max_size)

def buffer_init():

    print("初始化开始")

    init_path = Path("Experience/Init_Data.pth")

    if init_path.is_file():
        pass
        # buffer.load_from_file(init_path.as_posix())
        ...
        trajectory_reward_matrix = torch.from_numpy(trajectory_reward_matrix)
        o1_1_matrix = torch.from_numpy(o1_1_matrix)
        o2_1_matrix = torch.from_numpy(o2_1_matrix)
        o1_n1_matrix = torch.from_numpy(o1_n1_matrix)
        o2_n1_matrix = torch.from_numpy(o2_n1_matrix)
        state_1_matrix = torch.from_numpy(state_1_matrix)
        state_1_without_actions_matrix = torch.from_numpy(state_1_without_actions_matrix)
        state_n1_without_actions_matrix = torch.from_numpy(state_n1_without_actions_matrix)

        buffer.add_batch(trajectory_reward_matrix, o1_1_matrix, o2_1_matrix, o1_n1_matrix,
                           o2_n1_matrix, state_1_matrix, state_1_without_actions_matrix,
                           state_n1_without_actions_matrix)
        # buffer.save_to_file(init_path.as_posix())
...
for round in range(rounds):
    ...
    soft_update(critic_target_network1, critic1, tau=0.005)
    soft_update(critic_target_network2, critic2, tau=0.005)
    ...
    if gap_count == actor_update_gap:
        ...
        torch.nn.utils.clip_grad_norm_(actor1.parameters(), max_norm=0.1)
        torch.nn.utils.clip_grad_norm_(actor2.parameters(), max_norm=0.1)

        actor_network_optimizer1.step()
        actor_network_optimizer2.step()

        soft_update(actor_target_network1, actor1, tau=0.005)
        soft_update(actor_target_network2, actor2, tau=0.005)
    ...
    if (round + 1) % accumulate_reward_gap == 0:
        ...
        writer.add_scalar('Gradient/average_return', accumulate_reward, round+1)
        buffer_update(10000)
        ...
        if task_state == 0 and (round+1)/rounds > 0.75:
            break_flag = True
...
actor_path = "Model/actor1_02_26_No2.pth"
torch.save(actor1.state_dict(), actor_path)
actor_path = "Model/actor2_02_26_No2.pth"
