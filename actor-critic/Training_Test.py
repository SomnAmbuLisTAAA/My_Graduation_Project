...
unfinished_task = np.array( [[1],[1],[1],[1]], dtype=np.int32)
...
dis_agent2agent_mask = ~np.eye(agent_pos.shape[0], dtype=bool)

Actor_Network_1 = SequentialMultiLayerNN(3,256,4,2,0.6,2.5)
Actor_Network_2 = SequentialMultiLayerNN(3,256,4,2,0.6,2.5)

actor_path = "Model/actor1_02_25_No1.pth"
Actor_Network_1.load_state_dict(torch.load(actor_path, weights_only=True))
Actor_Network_1.eval()

actor_path = "Model/actor2_02_25_No1.pth"
Actor_Network_2.load_state_dict(torch.load(actor_path, weights_only=True))
Actor_Network_2.eval()
...
while not done:
    ...
    with torch.no_grad():
        ...
        '''observation = np.hstack((agent_pos, dis_agent2task, np.tile(task_state_decimal, (agent_pos.shape[0], 1)),
                                 dis_agent2agent[dis_agent2agent_mask].reshape(dis_agent2agent.shape[0], -1).copy()), dtype=np.float32)'''
        observation = np.hstack((agent_pos,np.tile(task_state_decimal, (agent_pos.shape[0], 1)),),dtype=np.float32)
        print(observation)

        observation_tensor = torch.from_numpy(observation)

        u_o1_tensor = Actor_Network_1.with_scaled_tanh(observation_tensor[0].unsqueeze(0)).view(1,2)
        u_o2_tensor = Actor_Network_2.with_scaled_tanh(observation_tensor[1].unsqueeze(0)).view(1,2)
