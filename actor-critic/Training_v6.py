...
actor_network_1 = SequentialMultiLayerNN(observation_dim, 256,4, agent_action_dim, 0.6, 2.5)
actor_target_network_1 = SequentialMultiLayerNN(observation_dim, 256,4, agent_action_dim, 0.6, 2.5)
actor_network_optimizer_1 = optim.AdamW(actor_network_1.parameters(), lr=actor_learning_rate, weight_decay=weight_decay)
# actor_network_scheduler_1 = torch.optim.lr_scheduler.ReduceLROnPlateau(actor_network_optimizer_1, mode='max', patience=5, factor=0.9 ,threshold=1e-4, min_lr=1e-8, cooldown=10)
# actor_network_scheduler_1 = torch.optim.lr_scheduler.ExponentialLR(actor_network_optimizer_1, gamma=0.999)

actor_network_2 = SequentialMultiLayerNN(observation_dim, 256,4, agent_action_dim, 0.6, 2.5)
actor_target_network_2 = SequentialMultiLayerNN(observation_dim, 256,4, agent_action_dim, 0.6, 2.5)
actor_network_optimizer_2 = optim.AdamW(actor_network_2.parameters(), lr=actor_learning_rate, weight_decay=weight_decay)
# actor_network_scheduler_2 = torch.optim.lr_scheduler.ReduceLROnPlateau(actor_network_optimizer_2, mode='max', patience=5, factor=0.9 ,threshold=1e-4, min_lr=1e-8, cooldown=10)
...
with torch.no_grad():
    ...
    path = 'Experience/DataSet_02_19_No2_tensors.pth'
    ...
    for element_to_count in range(16):
        ...
        print(f"第 7 列中元素 {element_to_count} 的个数: {count}")
...
for epoch in range(epochs):
    ...
    for trajectory_reward_batch, o1_1_batch, o1_n1_batch, o2_1_batch, o2_n1_batch, state_1_batch, state_1_without_actions_batch, state_n1_without_actions_batch in dataloader:
        ...
        soft_update(actor_target_network_1, actor_network_1, tau=0.05)
        soft_update(actor_target_network_2, actor_network_2, tau=0.05)
        soft_update(central_critic_target_network, central_critic_network, tau=0.05)
...
critic_path = "Model/critic_02_19_No1.pth"
torch.save(central_critic_network.state_dict(), critic_path)
actor_path = "Model/actor_1_02_19_No1.pth"
torch.save(actor_network_1.state_dict(), actor_path)
actor_path = "Model/actor_2_02_19_No1.pth"
